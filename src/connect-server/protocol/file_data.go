...
func (this *FileData) Encode() (buffer []byte, err error) {
	...
	binary.BigEndian.PutUint64(destBuff, uint64(this.OffSet))
...
func (this *FileData) Decode(msgBytes []byte) (err error) {
	...
	var index int64 = 0
	this.FromUuid, err = UuidToString(msgBytes[index : index+16])
	if err != nil {
	...
	this.DestUuid, err = UuidToString(msgBytes[index : index+16])
	if err != nil {
		err = fmt.Errorf("FileData Decode Error: %v", err)
		return err
	}
	...
	length := uint8(msgBytes[index])
	index += 1
	this.FileName = string(msgBytes[index : index+int64(length)])
	index += int64(length)
	this.OffSet = int64(binary.BigEndian.Uint64(msgBytes[index : index+8]))
	index += 8

	if len(msgBytes) > 41 {
		this.Data = make([]byte, int64(len(msgBytes))-index)
